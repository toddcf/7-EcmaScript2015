...
var allParks = [new Park("Green Park", 1963, 2, 80), new Park("Chill Park", 1979, 5, 120), new Park("Tranquility Park", 1983, 19, 1239)];

// Construct the streets:
var allStreets = [new Street("Huntington Blvd", 1949, 14, 5),
// No size argument on Evergreen, just to test the default parameter:
new Street("Evergreen Street", 1951, 7), new Street("Alpine Road", 1999, 10, 2), new Street("Constellation Circle", 2001, 12, 1)];
...
function parksReport(p) {
	...
	var _calcAvg = calcAvg(ages),
	    _calcAvg2 = _slicedToArray(_calcAvg, 2),
	    totalAge = _calcAvg2[0],
	    avgAge = _calcAvg2[1];
...
function streetsReport(s) {
	...
	var treeLengths = s.map(function (el) {
		return el.length;
	});
	// Then we can calculate the average.

	var _calcAvg3 = calcAvg(treeLengths),
	    _calcAvg4 = _slicedToArray(_calcAvg3, 2),
	    totalLength = _calcAvg4[0],
	    avgLength = _calcAvg4[1];
